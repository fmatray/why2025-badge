...
static void event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (event_id == WIFI_EVENT_AP_STACONNECTED) {
    ...
    if (retry_num < STA_MAXIMUM_RETRY) {
      ...
      esp_wifi_connect();
      retry_num++;
      ESP_LOGI(TAG, "retry to connect to the AP (%d/%d)", retry_num,
               STA_MAXIMUM_RETRY);
...
void wifi_init(void) {
  ...
  ESP_ERROR_CHECK(esp_event_handler_register(
      WIFI_EVENT, WIFI_EVENT_HOME_CHANNEL_CHANGE, &event_handler, NULL));
  // ESP_ERROR_CHECK( esp_event_handler_register(IP_EVENT,
  // IP_EVENT_AP_STAIPASSIGNED, &event_handler, NULL) );
...
bool start_wifi_ap(void) {
  ...
  const esp_timer_create_args_t timer_args = {
      .callback = &inactivity_timer_callback, .name = "inactivity-timer"};

  ESP_ERROR_CHECK(esp_timer_create(&timer_args, &inactivity_timer));

  ESP_LOGI(TAG, "WIFI_MODE_AP started. SSID:%s password:%s", AP_WIFI_SSID,
           AP_WIFI_PASSWORD);
...
bool start_wifi_sta() {
  ...
  int bits = xEventGroupWaitBits(wifi_event_group, CONNECTED_BIT, pdFALSE,
                                 pdTRUE, STA_TIMEOUT_MS / portTICK_PERIOD_MS);
  ESP_LOGI(TAG, "bits=%x", bits);
  if (bits & CONNECTED_BIT) {
    ESP_LOGI(TAG, "WIFI_MODE_STA connected. SSID:%s password:%s", STA_WIFI_SSID,
             STA_WIFI_PASSWORD);
